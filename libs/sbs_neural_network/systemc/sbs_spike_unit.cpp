...
#pragma HLS pipeline
          channel.data =
           (~(((ap_uint<CHANNEL_WIDTH> )  0xFFFF) << (SPIKE_VECTOR_WIDTH * (ip_index & 1))) & channel.data)
          | (((ap_uint<CHANNEL_WIDTH> ) spikeID) << (SPIKE_VECTOR_WIDTH * (ip_index & 1)));

          if ((ip_index & 1) || (ip_index == layerSize - 1))
          {
#pragma HLS pipeline
